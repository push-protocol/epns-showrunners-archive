        ...
        .then(payload => {
          ...
          ipfs.add(jsonisedPayload)
            .then(ipfshash => {
              ...
              const walletAddress = ethers.utils.computeAddress(config.ethTickerPrivateKey);
              ...
              let wallet = new ethers.Wallet(config.ethTickerPrivateKey, provider);

              // define contract
              let contract = new ethers.Contract(config.deployedContract, config.deployedContractABI, provider);
              // logger.info("Contract defined at address: %s with object: %o", ethers.utils.computeAddress(config.ethTickerPrivateKey), contract);

              // connect as a signer of the non-constant methode
              let contractWithSigner = contract.connect(wallet);
              let txPromise = contractWithSigner.sendMessage(ethers.utils.computeAddress(config.ethTickerPrivateKey), parseInt(payload.data.type), ipfshash, 1);
